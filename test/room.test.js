const { CONSTRAINTS } = require('../src/consts');
...
describe('Room Functions', () => {
    ...
    describe('isValidDimension', () => {
        ...
        it('should return false if value is less than min', () => {
            expect(isValidDimension(CONSTRAINTS.width.min - 1, 'width')).toBe(false);
        });

        it('should return false if value is greater than max', () => {
            expect(isValidDimension(CONSTRAINTS.width.max + 1, 'width')).toBe(false);
        });

        it('should return false if value is less than min', () => {
            expect(isValidDimension(CONSTRAINTS.height.min - 1, 'height')).toBe(false);
        });

        it('should return false if value is greater than max', () => {
            expect(isValidDimension(CONSTRAINTS.height.max + 1, 'height')).toBe(false);
        });
    ...
    describe('validateAndConfirmValue', () => {
        it('should validate and confirm value', async () => {
            ...
            const result = await validateAndConfirmValue(CONSTRAINTS.width.max + 1, 'width');
    ...
    describe('getRoomDimensions', () => {
        ...
        it('should re-prompt for dimensions if dimensions are invalid', async () => {
            getInput
                .mockResolvedValueOnce(`10 ${CONSTRAINTS.height.max + 1}`)
        ...
        it('should re-prompt for individual dimensions if dimension is invalid', async () => {
            getInput
                .mockResolvedValueOnce(`${CONSTRAINTS.width.min - 1} ${CONSTRAINTS.height.max + 1}`)
